...
a realistic, internationally-relevant domain. Feel free to clone, run, and study it.
...
- **No framework abstractions by design:** the retrieval pipeline, chunking, embedding calls, and metrics are implemented directly against the provider APIs — no LangChain, Semantic Kernel, or similar. The goal was to understand the mechanics at the root level before reaching for convenience layers.
- **Single initial commit:** the project was developed locally as a study piece before the decision to publish it. The history reflects that — it landed on GitHub as a finished snapshot rather than incrementally.
